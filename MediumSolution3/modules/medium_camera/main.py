...
def send_image(frame,classifier_url):
    ...
    try:
        response = requests.post(classifier_url, headers = headers, data = frame)
...
def capture_image_send_message(video_src, classifier_url, hubManager):
    ...
    while(True):
        #image = webcam.read()
        with open('test.jpg', mode="rb") as test_image:

            d1 = datetime.datetime.now()
            try:
                prob, item = send_image(test_image,classifier_url)
